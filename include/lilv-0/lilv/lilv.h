...
*/
LILV_API bool
lilv_nodes_contains(const LilvNodes* LILV_NULLABLE nodes,
...
/**
   ...
   @param link_dir Directory of links to external files, or NULL.  Links will
   be made here to any external file referred to in plugin state.  In turn,
   these will be linked to from `save_dir` (like save_dir/file => link_dir/file
   => /some/file).  This allows several state snapshots to share links to
   external files, so an archive (created by naive tools like `tar`) will only
   contain one copy of the file.  This can be the same as `save_dir` to link
   directly from there (without sharing).  If NULL, the state will contain
   absolute paths to any any external files.
...
/**
   Get the bundle that contains `state`, if any.

   This will return a path for states loaded from the filesystem, or from an
   instance using lilv_state_new_from_instance() with a `save_dir`.  Note that
   this can return a directory for states that haven't been saved yet, so the
   returned path isn't guaranteed to be a valid bundle with a `manifest.ttl`.

   Returns NULL if the state isn't stored in a directory for whatever reason.
*/
LILV_API const char* LILV_NULLABLE
lilv_state_get_bundle_path(const LilvState* LILV_NONNULL state);
...
/**
   ...
   The given directory and its manifest.ttl will be created if necessary, and
   file references in state will be linked to any special directories given to
   lilv_state_new_from_instance() when it was created, if possible.  Links in
   the saved bundle to any of those directories will be relative, any other
   link (to external files) will be absolute.  The general idea is that all
   these directories are part of some greater session which should be
   relocatable.
